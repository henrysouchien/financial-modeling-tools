...
bootstrap_env.bootstrap(required=["EDGAR_API_KEY", "ADDIN_DISPATCH_API_KEY"])
if not os.environ.get("ADDIN_DISPATCH_API_KEY"):
...
def _validate_reconcile_mode(mode: str) -> str | dict:
  return _validate_reconcile_mode_impl(mode, reconcile_error_fn=_reconcile_error)


def _validate_reconcile_build_kwargs(build_kwargs: dict) -> dict | dict[str, Any]:
  signature = _MODEL_BUILD_SIGNATURE or inspect.signature(model_build)
  return _validate_reconcile_build_kwargs_impl(
    build_kwargs,
    coerce_json_dict_arg_fn=_coerce_json_dict_arg,
    signature=signature,
    reconcile_error_fn=_reconcile_error,
    inspect_module=inspect,
  )


def _validate_reconcile_file_path_match(file_path: str, build_kwargs: dict) -> dict | None:
  return _validate_reconcile_file_path_match_impl(
    file_path,
    build_kwargs,
    reconcile_error_fn=_reconcile_error,
    normalized_reconcile_path_fn=_normalized_reconcile_path,
  )


def _validate_inputs_field(entry: dict, where: str) -> dict | None:
  return _validate_inputs_field_impl(
    entry,
    where,
    reconcile_error_fn=_reconcile_error,
  )
...
def _validate_reconcile_diagnostic_report(diagnostic_report: dict) -> dict | dict[str, Any]:
  return _validate_reconcile_diagnostic_report_impl(
    diagnostic_report,
    diagnostics_checked=_RECONCILE_DIAGNOSTICS_CHECKED,
    reconcile_error_fn=_reconcile_error,
  )


def _flagged_reconcile_entries(diagnostics: dict[str, Any]) -> list[dict[str, Any]] | dict:
  return _flagged_reconcile_entries_impl(
    diagnostics,
    validate_inputs_field_fn=_validate_inputs_field,
  )


def _group_payload(ticker: str, key: tuple[str, str], entries: list[dict[str, Any]]) -> dict:
  return _group_payload_impl(
    ticker,
    key,
    entries,
    delta_by_year_fn=_delta_by_year,
    filing_pointer_fn=_filing_pointer,
  )


def _worst_severity(entries: list[dict[str, Any]]) -> str:
  return _worst_severity_impl(
    entries,
    severity_order=_RECONCILE_SEVERITY_ORDER,
  )


def _before_summary(entries: list[dict[str, Any]]) -> dict:
  return _before_summary_impl(
    entries,
    worst_severity_fn=_worst_severity,
    delta_by_year_fn=_delta_by_year,
  )


def _after_summary(report: dict | None, diagnostic_type: str, subtotal_name: str) -> dict:
  return _after_summary_impl(
    report,
    diagnostic_type,
    subtotal_name,
    serialize_diagnostic_report_fn=_serialize_diagnostic_report,
    entries_from_serialized_report_fn=_entries_from_serialized_report,
    worst_severity_fn=_worst_severity,
    delta_by_year_fn=_delta_by_year,
  )


def _diagnostic_group_cleared(report: dict | None, diagnostic_type: str, subtotal_name: str) -> bool:
  return _diagnostic_group_cleared_impl(
    report,
    diagnostic_type,
    subtotal_name,
    serialize_diagnostic_report_fn=_serialize_diagnostic_report,
...
def _metrics_by_tag_for_concept(
  ...
  return _metrics_by_tag_for_concept_impl(
    ...
    concept_id,
    year,
    metric_value_for_concept_fn=_metric_value_for_concept,
  )
...
def _best_value_for_target(
  ...
  return _best_value_for_target_impl(
    ...
    target,
    floor_abs_m,
    relative_distance_fn=_relative_distance,
  )
...
def _evaluate_reconcile_candidates(
  *,
  ...
  return _evaluate_reconcile_candidates_impl(
    ...
    candidate_concepts_by_key=_RECONCILE_CANDIDATE_CONCEPTS,
    delta_by_year_fn=_delta_by_year,
    current_values_from_diagnostic_inputs_fn=_current_values_from_diagnostic_inputs,
    target_value_for_candidate_fn=_target_value_for_candidate,
    metrics_by_tag_for_concept_fn=_metrics_by_tag_for_concept,
    best_value_for_target_fn=_best_value_for_target,
  )


def _current_registry_revision() -> str | dict:
  return _current_registry_revision_impl(
    get_registry_cache_fn=get_registry_cache,
    reconcile_error_fn=_reconcile_error,
  )


def _override_directory() -> Path:
  return _override_directory_impl(schema_overrides_module=schema_overrides)


def _resolve_reconcile_override_path(ticker_upper: str) -> Path:
  return _resolve_reconcile_override_path_impl(
    ticker_upper,
    schema_overrides_module=schema_overrides,
    override_directory_fn=_override_directory,
  )
...
def _new_override_field(
  *,
  ...
  return _new_override_field_impl(
    ...
    delta_by_year_fn=_delta_by_year,
    tag_local_part_fn=_tag_local_part,
  )
...
def _override_conflict_response(
  ...
  return _override_conflict_response_impl(
    ticker,
    entries,
...
def _ticker_override_lock(ticker_upper: str):
  return _ticker_override_lock_impl(
    ticker_upper,
    override_directory_fn=_override_directory,
    fcntl_module=fcntl,
  )
